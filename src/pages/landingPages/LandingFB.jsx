...
const DynamicForm = ({ formPart, formApi, heading, description }) => {
  ...
  const handleClose = () => {
    setModalshow(false);
    navigate(`${basePath}/fb`); // when modal closes, go back to base
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "phone") {
      ...
      if (!newValue.startsWith("+1")) {
        newValue = "+1" + newValue.replace(/^\+?1?/, "");
      }

      // Remove non-digit characters except + at the start
      newValue = "+1" + newValue.slice(2).replace(/\D/g, "");

      // Limit to 10 digits after +1
      if (newValue.length > 12) {
        newValue = newValue.slice(0, 12); // +1 + 10 digits
      }
  ...
  const validateForm = () => {
    for (let field of formPart) {
      // Skip conditional field if not shown
      if (
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      property_own:
        formData.property_own === "Others"
          ? formData.property_own_other || ""
          : formData.property_own || "",
    };

    // console.log("payload: ", payload);
    // console.log("fbApi endpoint:", formApi);
    ...
    try {
      ...
      setModalshow(true);
      navigate(`${basePath}/fb/thank-you`);
    } catch (error) {
